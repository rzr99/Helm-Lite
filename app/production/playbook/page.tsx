            ...
            {STATIONS.map((s, i) => (
              <li key={s.key}>
                <Link
                  ...
                  className="flex items-start gap-4 px-5 py-4 transition-colors hover:bg-[var(--sunken)]"
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block text-sm text-[var(--text)]/55">
                      {s.objective}
                    </span>
                    {s.skipFor && s.skipFor.length > 0 && (
                      <span className="mt-1 block text-xs text-[var(--text)]/35">
                        Skipped for {s.skipFor.map(jobTypeLabel).join(", ")}
                      </span>
                    )}
                  </span>
                  <span className="mt-0.5 text-[var(--text)]/25">→</span>
                </Link>
              </li>
            ))}
          </ol>
